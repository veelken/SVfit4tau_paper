...
$\mathcal{P}(\bm{p}^{\vis(1)},\bm{p}^{\vis(2)},\bm{p}^{\vis(3)},\bm{p}^{\vis(4)};\pX^{\rec},\pY^{\rec}|m_{\textrm{X}})$.
in Eq.~(\ref{eq:likelihood_with_hadRecoil_final}).
Our algorithm evaluates the integral in Eq.~(\ref{eq:likelihood_with_hadRecoil_final}) for a series of mass hypotheses $m_{\textrm{X}^{(i)}$.
